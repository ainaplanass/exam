...
- ✅ **Buen ejemplo:** Interfaz `ICommunicable` permite nuevos animales sin modificar código
...
- ❌ **Mal ejemplo:** Interfaz grande `IWorker` obliga a implementar métodos no usados
- ✅ **Buen ejemplo:** Interfaces específicas (`IWorkable`, `IEatable`, `ISleepable`)
...
- ✅ **Buen ejemplo:** `OrderService` depende de interfaz `IDatabase`
